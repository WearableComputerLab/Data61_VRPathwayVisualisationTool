...
public class Locomotion : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Vector2 input = joystick.GetAxis(controller);
        //Vector3 movement = headTransform.TransformDirection(input.x, 0f, input.y);
        Vector3 movement = player.transform.TransformDirection(input.x, 0f, input.y);
        movement.y = 0;
        movement = movement.magnitude == 0 ? Vector3.zero : (movement / movement.magnitude);
        movement *= Time.deltaTime * (teleportAction.GetState(controller) ? SPRINT_SPEED : WALK_SPEED);
        //headTransform.Translate(movement);
